    ...
    public class IdleState : IState<IdleStateData>
    {
        ...
        public void Update()
        {
            if (!IsPointerOverUIElement())
            {
                ...
                GameObject hoveredTower = GetTowerByMousePosition(mousePosition);
                GameObject hoveredEnemy = GetEnemyByMousePosition(mousePosition);

                ChangeCursor(hoveredTower != null || hoveredEnemy != null);

                if (Input.GetMouseButtonDown(0))
                {
                    if (hoveredTower != null)
                    {
                        HandleTowerClicked(hoveredTower);
        ...
        private void HandleTowerClicked(GameObject tower)
        {
            _playerStates.SwitchState<TowerInfoState, TowerInfoStateData>(new TowerInfoStateData(tower));
        }
        ...
        {
            return _mapModel.GetTowerByPosition(position);
        }
