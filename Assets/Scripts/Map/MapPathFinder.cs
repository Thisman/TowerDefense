...
{
    public List<Vector3> GenerateEnemiesPath(MapModel mapModel)
    {
        // Convert world positions to grid positions
        Vector3 startPosition = mapModel.EnemiesSpawnPosition.transform.position;
        Vector3 targetPosition = mapModel.EnemiesTargetPosition.transform.position;
        Vector3Int startGridPos = mapModel.Map.WorldToCell(startPosition);
        Vector3Int targetGridPos = mapModel.Map.WorldToCell(targetPosition);
        ...
        Node startNode = new(startGridPos, mapModel.Map.GetCellCenterWorld(startGridPos), null, 0, GetHeuristic(startGridPos, targetGridPos));
        ...
        while (openSet.Count > 0)
        {
            ...
            {
                if (closedSet.Contains(neighbor) || !mapModel.IsTileWalkable(neighbor))
                {
                    continue;
                }
                ...
                {
                    neighborNode = new Node(neighbor, mapModel.Map.GetCellCenterWorld(neighbor), currentNode, gCost, GetHeuristic(neighbor, targetGridPos));
